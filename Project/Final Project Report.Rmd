...
All analysis was performed in R 4.1.0 (REF). Code for analysis and raw data can be accessed at (INSERT GITHUB LINK)

## Results

Females across all treatment groups had similar body masses, standard lengths, and condition factors ().

**Behavior**

Females exposed to only chemosensory stimuli showed a trend toward swimming farther distances than females exposed to only visual, multimodal, or no stimuli \@ref(fig:fig1). To quantify changes in females' association with the stimulus delivery site before and after stimulus delivery, we created an association index. Females exposed to only visual or multimodal stimuli showed a trend toward increasing their association with fry stimuli while females exposed to only chemosensory stimuli did not show the same trend (INSERT F VALUE, INSERT FIGURE REFERENCE).

(ref:captionFig1) Distance swam by females exposed to each sensory stimulus. Females exposed to only chemosensory stimuli showed a trend toward swimming more than females that received only visual or multimodal stimuli (p > 0.05). Boxes extend to the furthest data points of the first and third quartiles. Data median is represented by a solid line. Whiskers extend to the furthest data point within 1.5x the interquartile range. Outliers beyond 1.5x the interquartile range are represented by solid circles and are not reflective of statistical outliers.
...
Females exposed to multimodal stimuli had greater activation in subdivision 4 of the central dorsal telencephalon (Dc-4) compared to females exposed to unimodal or no stimuli and in subdivision 5 of the central dorsal telencephalon (Dc-5) compared to females exposed to no stimuli. In the periventricular posterior tuberculum (TPp), mouthbrooders exposed to multimodal stimuli showed greater neural activation than females exposed to unimodal or no stimuli (see Table 2 for post hoc results). 
...
As we continue this experiment, we will continue to collect behavioral and neural activation data from mothers, and collect behavioral and neural activation data from fry that are exposed to the same suite of stimuli from their mothers. As neural activation data from more nuclei are collected, we will perform multivariate analyses (e.g. PCA, differential function analysis) to identify networks of nuclei that are important for *A. burtoni* parent offspring recognition. 
